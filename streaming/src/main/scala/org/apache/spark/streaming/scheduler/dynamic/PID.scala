...
private[streaming] class PID(
  ...
  def nextBatchSize: Long = {
    if (jobSetHistory.head.isEmpty) {
      ...
      val according = jobSetHistory.getLastJobId()
      if(according == lastJobId){
        (lastBacth._1 * 2).min(maxBatchSize)
      }
      else {
        lastJobId = according
        ...
        logError(s"accodingto:${lastJobId-1} LastBathSize:${lastBacth._1}  " +
          s"LastProcess:${lastBacth._2} Error:$error history:$historicalError " +
          s"dError:$dError next:$ans nextBatch:$nextbacth")
        nextbacth
      }
    }
  }
